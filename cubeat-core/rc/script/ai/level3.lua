...
function THINK_INTERVAL() return 450 end --ms
function MISSRATE()       return 10  end --percentage. 0 ~ 100
...
function ai_entry(self)
  ...
  if keycube:exist() and random(100) > 33 and
     enemy_map:garbage_left() + power <= 9 -- so opponent doesn't feel like they are being overpowered too much.
  then
    ...
    if self:cmdqueue_size() < 1 then
      if ground_cube_num >= capacity * 0.7 and not my_map:still_chaining() then
