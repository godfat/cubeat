...
local function shake(loop, sprite, x, y, dis, dur)
  local tween     = {}
  local tweenback = {}
  for i=loop,1,-1 do
    if i==loop then
      tweenback[i]= function(self) self:tween("Linear", "Pos2D", ffi.new("value2", x+dis, y), ffi.new("value2", x-dis, y), dur, 0,
                      function(self) self:tween("Linear", "Pos2D", ffi.new("value2", x-dis, y), ffi.new("value2", x, y), dur/2, 0) end)
                    end
    else
...
actor_.slide_in = function(t)
  ...
  t.menu[t.actor]:set_visible(true)
  t.menu[t.actor]:tween("Linear", "Pos2D", ffi.new("value2", out_x, out_y), ffi.new("value2", act_x, act_y), config.slide_in_time)
end
...
actor_.slide_out = function(t)
  ...
  local out_y = config.out_y[t.ch]
  t.menu[t.actor]:set_visible(true)
  t.menu[t.actor]:tween("Linear", "Pos2D", ffi.new("value2", act_x, act_y), ffi.new("value2", out_x, out_y), config.slide_out_time)
end

actor_.fade_in = function(t)
  t.menu[t.actor]:set_visible(true)
  t.menu[t.actor]:tween("Linear", "Alpha", 0, 255, config.fade_in_time)
end

actor_.fade_out = function(t)
  t.menu[t.actor]:set_visible(true)
  t.menu[t.actor]:tween("Linear", "Alpha", 255, 0, config.fade_out_time)
end
...
actor_.shake = function(t)
  ...
  t.menu[t.actor]:set_visible(true)
  shake(loop, t.menu[t.actor], act_x, act_y, dis, dur)
end
...
word_.shake = function(t)
  ...
  t.menu[t.content]:set_scale( config.word_size_L )
  shake(loop, t.menu[t.content], con_x, con_y, dis, dur)
end
