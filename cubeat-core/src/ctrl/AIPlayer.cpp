...
bool AIPlayer::startThinking()
{
    if( !think_thread_ ) {
        ...
        if( ability_kind_ == 1 ) think_interval_ *= 0.93;
...
void AIPlayer::shoot(int x, int y, int type) //we must know ViewSetting here.
{
    ...
    int handshaking_x = utils::random(static_cast<int>(dist*0.05)) - static_cast<int>(dist*0.025);
    int handshaking_y = utils::random(static_cast<int>(dist*0.05)) - static_cast<int>(dist*0.025);
