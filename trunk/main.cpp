...
void step1(view::pSprite&);  //prototype
...
int main()
{
    ...
    view::pScene guiv = view::Scene::create();
    guiv->setTo2DView(640,480);

    view::pScene worldv = view::Scene::create();
    worldv->setTo3DView( PI/3.0f );

    view::pMap map1 = view::Map::create(0, worldv);
    map1->moveTo(-20,0,50);

    view::pCube cube0 = view::Cube::create( map1 );

    view::pCube c = view::Cube::create( map1 );
    c->moveTo(10,0,0).set<RGBDiffuse>(127);

    view::pAnimatedSceneObject testcube2 = view::AnimatedSceneObject::create("ex_move", worldv);
    testcube2->moveTo(5,5,50).set<Scale>(vector3df(0.3f, 0.3f, 0.3f))
                             .tween<Linear, Frame>(4801.0f,9600.0f,1500);

    view::pAnimatedSceneObject testcube3 = view::AnimatedSceneObject::create("ex_move", worldv);
    testcube3->moveTo(-5,-5,60).set<Scale>(vector3df(0.3f, 0.3f, 0.3f))
                               .tween<Linear, Frame>(0.0f,9600.0f,3000);

    view::pSprite cursor1 = view::Sprite::create( guiv );
    view::pSprite cursor2 = view::Sprite::create( guiv );
    cursor2->setDepth(6.0f);

    view::pButton something = view::Button::create( guiv );
    something->moveTo(100,100).set<RGBDiffuse>(128);

    something->onPress( &input2.trig1() ) = test;

    view::pSprite anotherthing = view::Sprite::create( guiv );
    anotherthing->moveTo(0,0);

    //newly added
    view::pAnimatedSprite girl = view::AnimatedSprite::create("test", guiv );
